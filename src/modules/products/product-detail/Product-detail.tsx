...
const ProductDetail = ({ history, products, shoppingCart }: any) => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    const idCurrentProduct = (history.location.pathname as string).split(
      "/"
    )[2];
  ...
  useEffect(() => {
    if ((products.products as any[]).length === 0 || !dataProduct.Id_Producto)
      return;
    const relatedProducts = products.products
      .filter(
        (item) =>
          item.Sexo === dataProduct.Sexo &&
          item.Id_Producto !== dataProduct.Id_Producto &&
          item.Tipo_Producto == dataProduct.Tipo_Producto
      )
      .slice(0, 3);
  ...
  const getProductDetail = (id: string) => {
    PbyService.getProductDetail(id).then((response: any[]) => {
      ...
      const images = response[0].Images
        ? (response[0].Images as string).split(",").map((item) => item.trim())
        : [];
  ...
  const addProductToCart = (sizeSelected) => {
    const inxFind = shoppingCart.products.findIndex(
      (item) =>
        item.Id_Producto === dataProduct.Id_Producto &&
        item.Talla === sizeSelected.Talla
    );
    if (inxFind >= 0) {
    ...
    toast.dark(
      `El producto ${dataProduct.Nombre_Producto} se ha agredado al carrito de compras`
    );
  };
  ...
  return (
    <div className={styles.product_detail_container}>
      ...
      >
        <img
          src="https://www.pbyclothing.com/recursos/images/TALLAS.png"
          alt=""
      ...
      <Breadcrumbs
        ...
        style={{ margin: "1.8em 0 1.8em 0" }}
      >
        {/* <Link color="inherit" href="/#" >{dataProduct.Nombre_Coleccion}</Link> */}
        <NavLink
          to={`/${dataProduct.Sexo?.toLowerCase()}`}
          onClick={(e) => {
      ...
      <div className={styles.product_detail}>
          ...
          <div className={styles.list_images}>
            {productImages.map((item, i) => (
              <img
                ...
                src={item}
                alt={dataProduct.Nombre_Producto || "" + i + 1}
              />
            ))}
          </div>
        </div>
        ...
        <div className={styles.product_info}>
          ...
          <div className={styles.price}>
            <strong
              style={{
                textDecoration: dataProduct.Aplica_Descuento
                  ? "line-through"
                  : "",
              }}
            >
          ...
          <div className={styles.main_data}>
            ...
            <br />
            {dataProduct.Descripcion_Producto ? (
          ...
          <div className={styles.buttons}>
            <Button
              ...
              onClick={() => addProductToCart(sizeSelected)}
            >
              Agregar al carrito
            </Button>
      ...
      <div className={styles.related_productsMobile}>
        <ProductList
          list={relatedProductList.slice(0, 2)}
