...
import { ProductsFeatured, Slider, SmallBanner } from "../../components";
...
function Home(props) {
  ...
  const [productsFeatured, setProductsFeatured] = useState<any[]>([]);
  const [bannerTexts, setBannerTexts] = useState<string[]>([]);
    ...
    if (products.length > 0) {
      const productsFeatured = products
        .sort((productA, productB) => {
          return (
            (new Date(productB.Fecha_Creacion) as any) -
            (new Date(productA.Fecha_Creacion) as any)
          );
        })
        .slice(0, 4);
      setProductsFeatured(productsFeatured);
    }
  }, [products]);

  const getItemsHomeBanner = () => {
    const items = (articles as any[]).filter(
      (item) => item.Codigo_Contenido === "HBN"
    );
  ...
  const getItemsDestacado = () => {
    const items = (articles as any[]).filter(
      (item) => item.Codigo_Contenido === "DST"
    );
    setItemsDestacado(items);
  };

  const getItemContenido = () => {
    const item = (articles as any[]).find(
      (item) => item.Codigo_Contenido === "CMUL"
    );
    setItemContenido(item);
  };

  const getBannerDestacadoContenido = () => {
    const item = (articles as any[])
      .filter((item) => item.Codigo_Contenido === "DES")
      .map((item) => item.Descripcion_Articulo as string);
    setBannerTexts(item);
  };
  ...
  return (
    <Fragment>
      ...
      <div className={styles.video_container}>
        <ReactPlayer
          url={itemContenido.Contenido_Multimedia}
          width={"100%"}
          height={"auto"}
          playing
          muted
